        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border-4">
            ...
            @foreach($usuarios as $usuario)
                ...
                <tr>
                    ...
                    <td class="flex">  <a href="{{route('editar.index', $usuario->id)}}"
                        ...
                        <form method="post" action="{{route('eliminar.index', $usuario->id)}}">
                            ...
                            {{method_field('DELETE')}}
                            <input type="submit" value="Delete" class="text-white cursor-pointer bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                        </form>
                    </td>
                </tr>
            @endforeach
